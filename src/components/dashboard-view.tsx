...
function hours(mins: number) { return `${(mins / 60).toFixed(mins % 60 ? 1 : 0)}h`; }

export function DashboardView({ data, onComplete, onNavigate, onAsk }: { data: PlannerData; onComplete: (session: StudySession) => void; onNavigate: (view: "calendar" | "tasks" | "assistant") => void; onAsk: (question: string) => void }) {
  ...
  const weekMins = weekSessions.reduce((sum, session) => sum + (session.status === "COMPLETED" ? session.actualMins ?? minutesBetween(session.startsAt, session.endsAt) : minutesBetween(session.startsAt, session.endsAt)), 0);
  ...
  return <div className="view dashboard-view">
    ...
    <div className="view-heading"><div><h1>Good afternoon, Maya <span>✦</span></h1><p>Here’s a clear look at what matters most today.</p></div><button className="icon-button notification" aria-label="View notifications"><Icon name="bell" size={19} /><i /></button></div>
    ...
    <section className="stats-grid" aria-label="Weekly statistics">
      <div className="metric-card accent-violet"><span className="metric-icon"><Icon name="clock" size={19} /></span><div><span>Study time this week</span><strong>{hours(weekMins)}</strong><small>+1.5h from last week</small></div></div>
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel workload-panel"><div className="panel-heading"><div><h2>Weekly workload</h2><p>Planned study time</p></div><span className="chart-total">{hours(weekMins)}</span></div><div className="bar-chart">{dailyHours.map((value, index) => <div className="bar-day" key={index}><div className="bar-track"><div className={`bar-fill ${index === (new Date().getDay() + 6) % 7 ? "today" : ""}`} style={{ height: `${Math.max(value ? 16 : 0, Math.min(100, value / 4 * 100))}%` }}><span>{value ? value.toFixed(value % 1 ? 1 : 0) : ""}</span></div></div><small>{["M", "T", "W", "T", "F", "S", "S"][index]}</small></div>)}</div><div className="chart-legend"><span><i className="legend-dot violet" /> Planned</span><span>Target: 12h</span></div></section>
