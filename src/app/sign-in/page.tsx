...
export default function SignInPage() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const result = await signIn("credentials", { redirect: false, redirectTo: "/", email: form.get("email"), password: form.get("password") });
    if (!result?.ok || result.error || !result.url) { setError("That email and password combination didn’t match."); setLoading(false); return; }
