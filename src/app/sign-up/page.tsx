...
export default function SignUpPage() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await signIn("credentials", { redirect: false, redirectTo: "/", email, password });
      if (!result?.ok || result.error || !result.url) { window.location.assign("/sign-in?created=1"); return; }
