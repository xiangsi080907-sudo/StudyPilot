...
export function AppShell({ initialData, mode, userName, loadError }: { initialData: PlannerData; mode: WorkspaceMode; userName: string; loadError?: string }) {
  ...
  if (activeView === "dashboard") content = <DashboardView data={data} userName={userName} onComplete={completeSession} onNavigate={changeView} onAsk={ask} />;
  ...
  else if (activeView === "assistant") content = <AssistantView key={assistantPrompt} data={data} mode={mode} initialQuestion={assistantPrompt} onQuestionUsed={() => setAssistantPrompt("")} />;
